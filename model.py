...
from torch_geometric.nn import GCNConv, SGConv, GATConv, JumpingKnowledge, APPNP, GCN2Conv, MessagePassing, GraphSAGE
...
class GloGNN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_nodes, dropout=0.5, alpha=0.5, beta1=0.5, beta2=0.5, gamma=0.1, norm_func_id=1, norm_layers=1, orders=1, orders_func_id=1):
        super(GloGNN, self).__init__()
        
        self.fc1 = nn.Linear(in_channels, hidden_channels).to(torch.float64)
        self.fc2 = nn.Linear(hidden_channels, out_channels).to(torch.float64)
        self.fc3 = nn.Linear(hidden_channels, hidden_channels).to(torch.float64)
        self.fc4 = nn.Linear(num_nodes, hidden_channels).to(torch.float64)
        ...
        self.orders = orders 
        self.register_buffer('class_eye', torch.eye(self.nclass, dtype=torch.float64))
        self.orders_weight = nn.Parameter(
            (torch.ones(orders, 1) / orders), requires_grad=True
        ).to(torch.float64)
        self.orders_weight_matrix = nn.Parameter(
            torch.DoubleTensor(out_channels, orders), requires_grad=True
        ).to(torch.float64)
        self.orders_weight_matrix2 = nn.Parameter(
            torch.DoubleTensor(orders, orders), requires_grad=True
        ).to(torch.float64)
        self.diag_weight = nn.Parameter(
    ...
    def reset_parameters(self):
        ...
        nn.init.kaiming_normal_(self.orders_weight_matrix, mode='fan_out')
        nn.init.kaiming_normal_(self.orders_weight_matrix2, mode='fan_out')
        self.diag_weight = nn.Parameter(
    ...
    def forward(self, data):
        x = data.graph['node_feat']
        edge_index = data.graph['edge_index'].long() 
        A = torch.zeros((x.size(0), x.size(0)), dtype=torch.float64, device=x.device)
